...
| D11 | Packages build the plugin **standalone against installed server headers**; the server-tree build (`link-plugin.sh` + `MYSQL_ADD_PLUGIN`) stays the developer path, and CI builds both and diffs the module's undefined-symbol set | `libmariadbd-dev` ships `mysql/plugin.h` and `mysql/service_sql.h` on every target — Debian 10.11, MariaDB.org 11.8, and both Homebrew kegs ([M9.1.1/M9.1.2](release-plan.md#m91--the-structural-decision-building-a-plugin-with-no-server-source-tree)) — so a package build needs no server source tree and cross-arch stops being a full server build under qemu. The symbol diff is the price: without it the packaged path drifts from the developer path silently, and the first person to notice is a user whose server won't start |
